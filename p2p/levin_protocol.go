...
//	binary.BigEndian.PutUint64(b, header.Signature)
	...
	binary.LittleEndian.PutUint32(b, header.ReturnCode)
	index += copy(headerBytes[index:], b[:4])
...
func ReceiveMessage() {
	...
	header.ReturnCode = binary.LittleEndian.Uint32(headerBytes[21:25])
	...
	cmd.data = data
	cmd.IsNotificatin = !header.HaveToReturnData
